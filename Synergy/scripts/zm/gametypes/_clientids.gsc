...
function initial_variable() {
	...
	self.syn["visions"][0] = strTok("AC-130|AC-130 inverted|Default Night|Night Vision|Endgame|Missile Cam|MP Intro|MP Nuke Aftermath", "|");
	self.syn["visions"][1] = strTok("ac130|ac130_inverted|default_night|default_night_mp|end_game|missilecam|mpintro|mpnuke_aftermath", "|");
	
	self.syn["weapons"]["category"][0] = strTok("assault_rifles|sub_machine_guns|sniper_rifles|shotguns|light_machine_guns|pistols|launchers|melee", "|");
	self.syn["weapons"]["category"][1] = strTok("Assault Rifles|Sub Machine Guns|Sniper Rifles|Shotguns|Light Machine Guns|Pistols|Launchers|Melee", "|");
	...
	self.syn["weapons"]["melee"][0] =              strTok("knife_loadout|", "|");
	self.syn["weapons"]["wonder"][0] =             strTok("ray_gun|", "|");
	...
	self.syn["weapons"]["melee"][1] =              strTok("Combat Knife|", "|");
	self.syn["weapons"]["wonder"][1] =             strTok("Ray Gun|", "|");
	
	self.syn["weapons"]["nzf"][0] =                strTok("tesla_gun|", "|");
	self.syn["weapons"]["nzf"][1] =                strTok("Wunderwaffe|", "|");
	...
	self.syn["perks"]["common"][0] = strTok("specialty_quickrevive|specialty_armorvest|specialty_doubletap2|specialty_staminup|specialty_fastreload|specialty_additionalprimaryweapon|specialty_deadshot|specialty_widowswine", "|");
	self.syn["perks"]["common"][1] = strTok("Quick Revive|Juggernog|Double Tap|Stamin-Up|Speed Cola|Mule Kick|Deadshot|Widow's Wine", "|");
...
function on_player_spawned() {
	...
	self thread create_text("SyndiShanX", "default", 2, "TOP_LEFT", "CENTER", 0, 0, "rainbow", 1, 3);
	...
	switch(menu) {
		...
		case "Account Options":
			...
			if(self.pers["plevel"] > 10) {
				self add_increment("Set Level", &set_rank, 1, 1, 1000, 1);
			} else {
				self add_increment("Set Level", &set_rank, 1, 1, 35, 1);
			}
			...
			//
			//self add_option("Give All Map Artifacts", &unlock_artifacts);
			//self add_option("Unlock " + self.syn["maps"][level.mapName] + " Artifact", &unlock_artifact);
		...
		case "Give Perks":
			...
			//		self add_option(self.syn["perks"][map][1][i], &give_perk, self.syn["perks"][map][0][i], 0);
			//	}
			//}
		...
		case "Take Perks":
			...
			//if(map == "cp_zmb" || map == "cp_disco" || map == "cp_town" || map == "cp_final") {
			//	for(i = 0; i < self.syn["perks"][map][0].size; i++) {
			//		self add_option(self.syn["perks"][map][1][i], &take_perk, self.syn["perks"][map][0][i], 0);
			//	}
			//}
		...
		case "Assault Rifles":
			...
			for(i = 0; i < self.syn["weapons"][category][0].size; i++) {
				self add_option(self.syn["weapons"][category][1][i], &give_weapon_to_player, self.syn["weapons"][category][0][i]);
			}
...
function test_function() {
	test_weapon = self getCurrentWeapon();
	iPrintString("Test Debug Print");
	wait 1;
	iPrintString(test_weapon[0]);
	//zm_spawner::do_zombie_spawn();
	//zm_powerups::powerup_drop(self.origin);
	//wait 2.5;
	//iPrintString(self zm_utility::get_player_melee_weapon());
	//zm_melee_weapon::give_ballistic_knife(1, 1);
	//_zm_sidequests::sidequest_complete(sidequest_name);
}
...
function get_position_loop() {
	...
	for(;;) {
		if(!isDefined(self.syn["position"])) {
			self.syn["position"][0] = self create_text(self.origin + " | " + self.angles, "default", 1, "TOP_LEFT", "CENTER", (self.syn["utility"].x_offset + 100), (self.syn["utility"].y_offset - 500), (0,1,1), 1, 9999, false, true); // Player Origin
		} else {
...
function get_map_name() {
  if(level.script == "zm_prototype") return "nzp";		//Nacht der Untoten
  if(level.script == "zm_asylum") return "nza";				//Verruckt
  if(level.script == "zm_sumpf") return "nzs";				//Shi No Numa
  if(level.script == "credits") return "cred";				//Credits
  if(level.script == "zm_factory") return "nzf";			//The Giant
  if(level.script == "zm_castle") return "de";				//Der Eisendrache
  if(level.script == "zm_island") return "zns";				//Xetsubou No Shima
  if(level.script == "zm_genesis") return "rev";			//Revelations
  if(level.script == "zm_stalingrad") return "gk";		//Gorod Krovi
  if(level.script == "zm_zod") return "soe";					//Shadows of Evil
  if(level.script == "zm_tomb") return "origins";			//Origins
  if(level.script == "zm_moon") return "moon";				//Moon
  if(level.script == "zm_cosmodrome") return "ascen";	//Ascension
  if(level.script == "zm_theater") return "kino";			//Kino der toten
  if(level.script == "zm_temple") return "shang";			//Shangri-la
}
...
function get_perk_name(perk) {
	for(i = 0; i < self.syn["perks"]["common"][0].size; i++) {
		if(perk == self.syn["perks"]["common"][0][i]) {
			...
			iPrintString(perk + " != " + self.syn["perks"]["common"][0][i]);
			wait 1;
			return perk;
		}
	}
...
function freeze_box() {
	...
	if(self.freeze_box) {
		...
		iPrintString(level.chest_min_move_usage);
		wait 5;
...
function give_weapon_to_player(weapon) {
	weapon = getWeapon(weapon);
	
	if(isDefined(self.give_packed_weapon) && self.give_packed_weapon == 1) {
		if(self.give_packed_weapon == 1) {
			iPrintString("Packed Weapon Code goes Here");
		}
	}
	
	if(self getCurrentWeapon() != weapon && self getWeaponsListPrimaries()[1] != weapon && self getWeaponsListPrimaries()[2] != weapon && self getWeaponsListPrimaries()[3]) {
		max_weapon_num = 2;
...
//function spawn_zombie(archetype, team) {
...
function freeze_zombies() {
	if(!isDefined(self.freeze_zombies)) {
		...
		level.prev_speed = level.zombie_move_speed;
		...
		level.zombie_move_speed = level.prev_speed;
	}
}
